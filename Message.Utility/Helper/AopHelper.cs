...
namespace Messag.Utility.AopHelper
{
    /// <summary>
    ///AopHelper
    /// </summary>
    ...
    public static class AopHelperExtensions
    {
        ...
        {
            logger = new Logger.Logger("AopHelper");
        }
        ...
        public static AopHelper Log(this AopHelper aopHelper, string msg)
        {
            ...
            {
                logger.Info(String.Format("{0}开始:{1}", msg, DateTime.Now));

                work();

                logger.Info(String.Format("{0}结束:{1}", msg, DateTime.Now));
            });
        }
        ...
        public static AopHelper CalcExecuteTime(this AopHelper aopHelper, string method)
        {
            ...
            return aopHelper.Combine((work) =>
            {
                ...
                logger.Info(String.Format("{0}执行了{1}毫秒", method, milliseconds));
            });
        }
        ...
        {
            logger.Info("数据库重试最终失败!");
        }

        public static void TryException(params object[] whatever)
        {
            logger.Info(String.Format("数据库操作异常：{0}", whatever));
        }


        public static void DoNothing()
        {
            logger.Info("最终还是彻底失败了!");
        }

        public static void DoNothing(params object[] whatever)
        {
            logger.Info(String.Format("异常了,原因：{0}", whatever));
        }
