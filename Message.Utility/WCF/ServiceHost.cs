...
namespace Messag.Utility.WCF
{
    ...
    public class ServiceHost<C, I> : ServiceHost
    {
            ...
            : base(typeof(I), uris)
        {
            ...
            this.myChannelFactory = new ChannelFactory<C>(binding, myEndpoint);

            if (behaviors != null)
            {
                behaviors.ToList().ForEach(b =>
                {
                    this.myChannelFactory.Endpoint.Behaviors.Add(b);
                    this.AddServiceEndpoint(typeof(C), binding, "").Behaviors.Add(b);
                });
            }

            this.Contract = this.myChannelFactory.CreateChannel();
        }
